...
int main(int argc, char *argv[]) {
  ...
  *num_turmas = 0;
  ...
  importar(clientes, num_clientes, argv[3]);     // chamada da função que importa os dados do ficheiro de texto e retorna o número de linhas do mesmo
...
void importar(Cliente *clientes, int *num_clientes, char nome_ficheiro[]){
  ...
  while(fgets(buffer, BUF_SIZE, f)){    // ler o ficheiro de texto até não haver mais nada para ler
    ...
    strcpy(nome, str);
    str = strtok(NULL, ";");      // pega na primeir sub-string depois de ";"
    strcpy(passe, str);
